...
def traink(tr_raw_mohx, val_raw_mohx, TOTAL_EPOCHS, device, model_dir, max_len, batch_size, learning_rate,
    ...
    if operation == 'train':
        model = MrBERT.from_pretrained(model_dir, config=config, bert_model_dir=model_dir,
...
def k_fold(k, raw_mohx, num_epochs, device, model_dir, max_len, batch_size, lr, repr, integrate, relmodel, operation):
    ...
    for i in range(k):
        ...
        val_acc, val_p, val_r, val_f1 = traink(*data, num_epochs, device, model_dir, max_len, batch_size, lr,
                                               repr, integrate, relmodel, operation)
...
def test(test_raw_mohx, device, max_len, repr, integrate, relmodel):
    ...
    model = MrBERT(bert_model_dir='./model', config=config, device=device, repr=repr, integrate=integrate,
                   relmodel=relmodel)
    checkpoint = torch.load('./model/pytorch_model.bin', map_location=device)
    model.load_state_dict(checkpoint)
    model.to(device)
...
def main():
    ...
    if args.operation == 'test':
        test(raw_mohx, device, args.max_len, args.repr, args.integrate, args.relmodel)
    elif args.operation == 'finetune':
        k_fold(args.kfold, raw_mohx, args.num_epochs, device, './model', args.max_len, args.batch_size,
