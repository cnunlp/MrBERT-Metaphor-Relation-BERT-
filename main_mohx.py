...
def load_mohx():
    ...
    with open('../MrBERT/data/MOH-X/mohx_labels.csv', encoding='utf8') as f:
        lines = csv.reader(f)
        next(lines)
        for line in lines:
            svo_labels.append(ast.literal_eval(line[0]))
    with open('../MrBERT/data/embeddings_mohx/mohx_embeddings_ave.csv', encoding='utf8') as f:
        lines = csv.reader(f)
        next(lines)
        embeddings = []
        for line in lines:
    ...
    raw_mohX = []
    with open('../MrBERT/data/MOH-X/MOH-X_formatted_svo_cleaned.csv', encoding='utf8') as f:
        lines = csv.reader(f)
        next(lines)
        i = 0
        for line in lines:
...
def traink(tr_raw_mohx, val_raw_mohx, TOTAL_EPOCHS, device, bert_base_model_dir, max_len, batch_size, learning_rate,
           repr, integrate, relmodel, operation):
    ...
    config = BertConfig.from_pretrained(bert_base_model_dir + '/config.json')
    config.num_labels1 = 2
    config.num_labels2 = 2

    if operation == 'train':
        model = MrBERT.from_pretrained(bert_base_model_dir, config=config, bert_model_dir=bert_base_model_dir,
                                       device=device, repr=repr,
                                       integrate=integrate, relmodel=relmodel)
    elif operation == 'finetune':
        model = MrBERT(bert_model_dir=bert_base_model_dir, config=config, device=device, repr=repr, integrate=integrate,
                       relmodel=relmodel)
    ...
    # ! get inputs
    tr_input_ids, tr_labels1, tr_labels2, tr_masks, tr_embeddings = get_inputs(tokenizer, tr_tokenized_texts,
                                                                               tr_labels0, tr_labels2, max_len,
                                                                               tr_embeddings)
    val_input_ids, val_labels1, val_labels2, val_masks, val_embeddings = get_inputs(tokenizer, val_tokenized_texts,
                                                                                    val_labels0, val_labels2, max_len,
                                                                                    val_embeddings)
...
def k_fold(k, raw_mohx, num_epochs, device, bert_base_model_dir, max_len, batch_size, lr, repr, integrate, relmodel, operation):
    val_acc_sum, val_p_sum = 0, 0
    val_r_sum, val_f1_sum = 0, 0
    for i in range(k):
        print('*' * 25, i + 1, 'fold', '*' * 25)
        data = get_kfold_data(k, i, raw_mohx)

        val_acc, val_p, val_r, val_f1 = traink(*data, num_epochs, device, bert_base_model_dir, max_len, batch_size, lr,
                                               repr, integrate, relmodel, operation)
...
def test(test_raw_mohx, device, bert_base_model_dir, max_len, repr, integrate, relmodel):
...
def main():
    ...
    # parser.add_argument('--mrbert_model_dir',default='./model', type=str, required=False)
    parser.add_argument('--max_len', default=30, type=int, required=False)
    ...
    if args.operation == 'test':
        test(raw_mohx, device, args.bert_base_model_dir, args.max_len, args.repr, args.integrate, args.relmodel)
    else:
        k_fold(args.kfold, raw_mohx, args.num_epochs, device, args.bert_base_model_dir, args.max_len, args.batch_size,
               args.lr, args.repr, args.integrate, args.relmodel, args.operation)
