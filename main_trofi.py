...
def load_trofi():
    ...
    with open('./data/TroFi/trofi_labels.txt', encoding='utf8') as f:
        for line in f:
            test_svo_labels.append(line.split())
    with open('./data/embeddings_trofi/trofi_embeddings_ave.csv', encoding='utf8') as f:
        lines = csv.reader(f)
        next(lines)
        test_embeddings = []
        for line in lines:
    ...
    raw_trofi = []
    with open('./data/TroFi/TroFi_formatted_all3737.csv', encoding='utf8') as f:
        lines = csv.reader(f)
        next(lines)
        i = 0
        for line in lines:
...
def get_kfold_data(k, i, raw_trofi):
    ...
    fold_size = len(raw_trofi) // k  # 每份的个数:数据总条数/折数（组数）
...
def traink(tr_raw_trofi, val_raw_trofi, TOTAL_EPOCHS, device, bert_base_model_dir, mrbert_model_dir, max_len, batch_size, learning_rate, max_grad_norm, repr, integrate, relmodel, operation):
    '''第k折 模型训练
    '''
    ...
    if operation == 'train':
        ...
        model = MrBERT(model_dir=bert_base_model_dir, config=config, device=device, repr=repr, integrate=integrate,
                       relmodel=relmodel)
...
def k_fold(k, raw_trofi, num_epochs, device, bert_base_model_dir, mrbert_model_dir, max_len, batch_size, lr, max_grad_norm, repr, integrate, relmodel, operation):
    '''k折
    '''
    ...
    for i in range(k):
        print('*' * 25, '第', i + 1, '折', '*' * 25)
        data = get_kfold_data(k, i, raw_trofi)  # 获取k折交叉验证的训练和验证数据
...
def test(test_raw_trofi, device, model_dir, max_len, repr, integrate, relmodel):
    ...
    # headers = ['sentence', 'labels','label','s_pos','v_pos','o_pos']
    # fname = 'result_trofi/trofi.csv'
    # with open(fname, 'w')as f:
    #     f_csv = csv.writer(f)
    #     f_csv.writerow(headers)
    #     f_csv.writerows(test_raw_trofi)
    ...
    model.load_state_dict(checkpoint)
    # model = MrBERT.from_pretrained(model_dir, config=config, device=device, model_dir=model_dir, repr=repr,
    #                                integrate=integrate, relmodel=relmodel)
    model.to(device)
    model.eval()
...
def main():
    ...
    parser.add_argument('--max_len', default=130, type=int, required=False, help='句子最大长度')
    ...
    if args.operation == 'test':
        test(raw_trofi, device, args.bert_base_model_dir, args.mrbert_model_dir, args.max_len, args.repr, args.integrate, args.relmodel)
    else:
        k_fold(args.kfold, raw_trofi, args.num_epochs, device, args.bert_base_model_dir, args.mrbert_model_dir, args.max_len, args.batch_size, args.lr, args.max_grad_norm, args.repr, args.integrate, args.relmodel, args.operation)
