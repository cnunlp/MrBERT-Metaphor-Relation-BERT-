...
def load_vua_test():
    ...
    with open('../MrBERT/data/VUA/VUA_seq_formatted_test.csv', encoding='utf8') as f:
        lines = csv.reader(f)
        next(lines)
        for line in lines:
...
def test(test_raw_vua, test_pos, device, max_len, repr, integrate, relmodel):
    # get data
...
def main():
    ...
    test(raw_vua_test, test_pos, device, args.max_len, args.repr, args.integrate,
         args.relmodel)
