...
class MrBERT(BertPreTrainedModel):
    def __init__(self, config, bert_model_dir, device, repr, relmodel):
    ...
    Returns:
        ...
        for i in range(len(input_ids)):
            ...
            f = torch.nn.Sigmoid()
            sim_subj = torch.cosine_similarity(subj_states.reshape(-1), verb_states.reshape(-1), dim=0)
            sim_obj = torch.cosine_similarity(obj_states.reshape(-1), verb_states.reshape(-1), dim=0)
            sim_cls = torch.cosine_similarity(cls_states.reshape(-1), verb_states.reshape(-1), dim=0)
            sim_embedding = torch.cosine_similarity(verb_embeddings.reshape(-1), verb_states.reshape(-1), dim=0)
            if s_flag and o_flag:
                sim = torch.softmax(torch.FloatTensor([sim_cls, sim_subj, sim_obj, sim_embedding]), 0)
                context_states = sim[0] * cls_states.float() + sim[1] * subj_states.float() + sim[2] * obj_states.float() + sim[3] * verb_embeddings.float()
            elif s_flag:
                sim = torch.softmax(torch.FloatTensor([sim_cls, sim_subj, sim_embedding]), 0)
                context_states = sim[0] * cls_states.float() + sim[1] * subj_states.float() + sim[2] * verb_embeddings.float()
            elif o_flag:
                sim = torch.softmax(torch.FloatTensor([sim_cls, sim_obj, sim_embedding]), 0)
                context_states = sim[0] * cls_states.float() + sim[1] * obj_states.float() + sim[2] * verb_embeddings.float()
            else:
                sim = torch.softmax(torch.FloatTensor([sim_cls, sim_embedding]), 0)
                context_states = sim[0] * cls_states.float() + sim[1] * verb_embeddings.float()
            if self.relmodel == 'linear' or self.relmodel == 'nt':
                relation_linear_score = self.relation_linear(
                    torch.cat((verb_states.float(), context_states), dim=-1).unsqueeze(0))
